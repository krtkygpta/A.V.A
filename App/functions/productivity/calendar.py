...
def _get_credentials():
    ...
    token_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "data", "calendar_token.json"
    )
    client_secrets = os.path.join(
        os.path.dirname(__file__), "..", "..", "data", "calendar_client_secrets.json"
    )
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not os.path.exists(client_secrets):
                return (
                    None,
                    "Client secrets file not found. Add 'calendar_client_secrets.json' to data/ folder",
                )
...
def calendar(operation: str, **kwargs):
    ...
    try:
        if operation == "list":
            ...
            return json.dumps(
                {"status": "error", "content": f"Unknown operation: {operation}"}
            )
...
def _list_events(service, max_results: int = 10, days_ahead: int = 7):
    ...
    try:
        ...
        if not events:
            return json.dumps(
                {"status": "success", "content": "No upcoming events found"}
            )
...
def _create_event(
    ...
    try:
        ...
        return json.dumps(
            {
                "status": "error",
                "content": "Invalid datetime format. Use ISO format (e.g., 2024-04-25T14:00:00)",
            }
        )
    except HttpError as e:
        return json.dumps({"status": "error", "content": f"HTTP Error: {e}"})
